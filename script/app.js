...
function basket() {
    ...
    for (let index = 0; index < productList.length; index++) {
        basketChecklist.innerHTML += carditemBureger(productList[index])
        
    }
const allCount = totalContProduct()
if (allCount) {
    basketCount.classList.add('active')
}else{
...
function carditemBureger(productData) {
    const {name, price, image, amount} = productData
    return `
    <div class="wrapper__navbar-product">
        <div class="wrapper__navbar-info">
            <img src="${image}" alt="${name}" class="wrapper__navbar-productImage">
